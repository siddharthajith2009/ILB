...
from flask import Flask, request, jsonify, render_template  # Flask for web app, request for getting data, jsonify for JSON responses, render_template for serving HTML
...
import openai  # For AI-powered explanations
...
def load_model_and_mappings():
    ...
    # Step 7: Initialize OpenAI API
    openai_api_key = os.getenv('OPENAI_API_KEY') or "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"
    if openai_api_key:
        openai.api_key = openai_api_key
        print("✓ OpenAI API configured!")
    else:
        print("⚠️  OpenAI API key not found. AI explanations will be disabled.")
        print("   Set OPENAI_API_KEY environment variable to enable AI explanations.")
    
    print("All models and mappings loaded successfully!\n")


def generate_prediction_explanation(job_title, education, experience_years, prediction, confidence, probabilities):
    """
    Generate an AI-powered explanation for why a job prediction was made
    """
    if not openai.api_key:
        return "AI explanations are currently unavailable. Please set the OPENAI_API_KEY environment variable."
    
    try:
        # Create a detailed prompt for the AI
        prompt = f"""
        You are an expert career analyst and labor market researcher. Explain why a job prediction was made based on the following data:
        ...
        {', '.join([f"{status}: {prob}%" for status, prob in probabilities.items()])}

        Please provide a clear, professional explanation (2-3 paragraphs) of why this job is predicted to be {prediction.lower()}. Consider:
        1. Current market trends for this role
        2. How education level affects job demand
        3. How experience level impacts job prospects
        4. Industry factors that influence job growth
        5. Economic and technological factors

        Be specific, factual, and helpful for someone making career decisions.
        """
        
        response = openai.ChatCompletion.create(
            model="gpt-3.5-turbo",
            messages=[
                {"role": "system", "content": "You are a helpful career analyst who provides clear, accurate explanations about job market predictions."},
        ...
        return f"Unable to generate AI explanation at this time. Error: {str(e)}"
...
def home():
    """
    Serve the main website home page
    Access this at: http://localhost:9000/
    """
    return render_template('index.html')

# ========== ROUTE 2: Search Page (GET) ==========
@app.route('/search', methods=['GET'])
def search():
    """
    Serve the search page for job predictions
    Access this at: http://localhost:9000/search
    """
    return render_template('search.html')

# ========== ROUTE 3: Results Page (GET) ==========
@app.route('/results', methods=['GET'])
def results():
    """
    Serve the results page
    Access this at: http://localhost:9000/results
    """
    return render_template('results.html')

# ========== ROUTE 4: Chatbot Page (GET) ==========
@app.route('/chatbot', methods=['GET'])
def chatbot():
    """
    Serve the AI chatbot page
    Access this at: http://localhost:9000/chatbot
    """
    return render_template('chatbot.html')

# ========== ROUTE 4: API Welcome Message (GET) ==========
@app.route('/api', methods=['GET'])
def api_home():
    """
    API documentation endpoint
    Access this at: http://localhost:9000/api
    """
    return jsonify({
        ...
        'endpoints': {
            ...
            '/api/options': 'GET - Get all available options',
            '/api/explain': 'POST - Generate AI explanation for predictions',
...
# ========== ROUTE 5: Prediction Endpoint (POST) ==========
...
# ========== ROUTE 6: Get Available Options (GET) ==========
...
# ========== ROUTE 7: Get Valid Combinations (POST) ==========
@app.route('/api/valid-options', methods=['POST'])
...
# ========== ROUTE 8: Generate AI Explanation (POST) ==========
@app.route('/api/explain', methods=['POST'])
def explain_prediction():
    """
    Generate an AI-powered explanation for a prediction
    """
    try:
        data = request.get_json()
        
        # Validate required fields
        required_fields = ['job_title', 'education', 'experience_years', 'prediction', 'confidence', 'probabilities']
        missing_fields = [field for field in required_fields if field not in data]
...
# ========== ROUTE 9: AI Chatbot (POST) ==========
@app.route('/api/chat', methods=['POST'])
def chat_with_ai():
    """
    Chat with AI assistant about career and job market insights
    """
    try:
        data = request.get_json()
        ...
        if not openai.api_key:
            return jsonify({
                'success': True,
                'response': "I'm currently unavailable. Please ensure the AI service is properly configured."
            })
        
        # Create a comprehensive prompt for career-focused AI
        system_prompt = """You are an expert AI Career Assistant specializing in job market trends, career advice, and employment insights. You help users understand:

1. Job market trends and predictions
2. Career growth opportunities
3. Skills development recommendations
4. Industry insights and changes
5. Salary expectations and career paths
6. Impact of technology on jobs
7. Remote work trends
8. Education and certification advice

You should provide:
- Accurate, up-to-date career information
- Practical advice for job seekers
- Insights about growing and declining job sectors
- Specific skill recommendations
- Industry-specific guidance
- Encouraging and supportive tone

Keep responses helpful, concise (2-3 paragraphs), and actionable. Focus on practical career advice that helps users make informed decisions."""
        ...
        try:
            ...
            return jsonify({
                'success': True,
                'response': "I'm having trouble processing your request right now. Please try again in a moment, or feel free to ask about job market trends, career advice, or skills development!"
            })
...
if __name__ == '__main__':
    ...
    print("\n" + "="*60)
    print("  JOB STATUS PREDICTION WEBSITE & API SERVER")
    print("="*60)
    ...
    print("  - GET  /chatbot      : AI chatbot page")
    print("  - GET  /api          : API documentation")
    print("  - POST /api/predict  : Make a prediction")
    print("  - GET  /api/options  : Get all available job titles and education levels")
    print("  - POST /api/valid-options: Get valid options based on selections (smart filtering)")
    print("  - POST /api/explain  : Generate AI-powered explanation for predictions")
